...
async def save_calibration(
    ...
    # 1. Process and save image to dataset/images/ as RGB JPEG
    unique_id = str(uuid.uuid4())
    img_filename = f"{unique_id}.jpg"
    img_path = os.path.join(IMAGES_DIR, img_filename)
    ...
    # 2. YOLOv8-Pose formatting logic
    x0, y0 = p0
    x12, y12 = p12
    ...
    # 3. Record mapping of UUID to original filename in dataset_mapping.json
    mapping_path = os.path.join(DATASET_DIR, "dataset_mapping.json")
    try:
        mapping = {}
        if os.path.exists(mapping_path):
    ...
    # 4. Save/update ground truth mapping in ruler_ground_truth.json
    gt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ruler_ground_truth.json")
    ground_truth = {}
    if os.path.exists(gt_path):
        try:
    ...
    ground_truth[filename] = {
        "p0": {"x": round(x0, 2), "y": round(y0, 2)},
        "p12": {"x": round(x12, 2), "y": round(y12, 2)},
        "rulerLengthMm": ruler_length_mm
    }
