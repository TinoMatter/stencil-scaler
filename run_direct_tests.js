...
function drawCalibrationLine(mat, p0, p12, reliable, snapMm = 120) {
  ...
  if (rulerLen > 10) {
    ...
    const cmLen = Math.max(12, Math.round(maxDim / 100)); // Length of 1 cm tick
...
function runAllTests() {
  ...
  const files = fs.readdirSync(STENCILS_DIR)
    .filter(f => !f.startsWith('.') && groundTruth[f] &&
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (file.endsWith('.pdf')) {
        meta = {
          isPdf: true, pageIndex: 1,
          pageBreitePt: width / 3,
          pageHöhePt: height / 3,
          renderScale: 3.0,
          sourceWidthPx: width,
      ...
      for (const algo of ACTIVE_ALGORITHMS) {
        ...
        try {
          ...
          // Draw verification status watermark/badge at the top-left of the preview
          const badgeText = isVerified ? "VERIFIED GROUND TRUTH" : "UNVERIFIED (NO GROUND TRUTH)";
          const badgeColor = isVerified ? new cv.Scalar(0, 180, 0, 255) : new cv.Scalar(255, 64, 64, 255);
          const maxDim = Math.max(preview.cols, preview.rows);
          const fontScale = Math.max(1.0, maxDim / 1500);
          const thickness = Math.max(2, Math.round(fontScale * 2.5));
          const badgeOrg = new cv.Point(Math.round(40 * fontScale), Math.round(80 * fontScale));
          // Outline for readability
          cv.putText(preview, badgeText, badgeOrg, cv.FONT_HERSHEY_SIMPLEX, fontScale, new cv.Scalar(255, 255, 255, 255), thickness + 4, cv.LINE_AA);
          cv.putText(preview, badgeText, badgeOrg, cv.FONT_HERSHEY_SIMPLEX, fontScale, badgeColor, thickness, cv.LINE_AA);

          const baseName = path.basename(file, path.extname(file));
          const versionDir = path.join(OUTPUT_DIR, algo.id);
          ...
          if (isVerified) {
            ...
            if (reliable) {
              ...
              const maxErrMm = finalErrPx / pxPerMm;

              if (maxErrMm > 2.0) {
                gtStatus = `FAIL (${maxErrMm.toFixed(2)}mm)`;
                gtPassedAll = false;
              } else {
                gtStatus = `OK (${maxErrMm.toFixed(2)}mm)`;
              }
            } else {
              gtStatus = 'FAIL (FALLBACK)';
              gtPassedAll = false;
            }
          } else {
            gtStatus = 'SKIP';
...
function writePerAlgorithmReports(rowsByVersion, filesCount) {
  for (const version of ALL_VERSIONS) {
    ...
    const reportPath = path.join(OUTPUT_DIR, version, 'report.md');
